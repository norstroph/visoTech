...
export const Get = async (entity: any, email: string) => {
    try {
        const user = await db.table(entity).get(email)
        if(user !== email){
            return true
        }
        return false
...
export const GetByEmail = async (entity: any, email: string) => {
    try {
        return await db.table(entity).where("email").equals(email).first()
    } catch (error) {
        console.log(error)
    }
}
